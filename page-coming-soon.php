...
<?php
	...
	$nonce = wp_create_nonce( 'zn_hg_mailchimp' );
?>
		...
		<div class="content offline-page-content">
			...
			<?php
				...
				if ( ! empty ( $cs_lsit_id ) && ! empty ( $mailchimp_api ) ) {
					...
					echo '		<form method="post" class="newsletter_subscribe newsletter-signup kl-newsletter" data-url="' . esc_url(home_url('/')) . '" name="newsletter_form">';
					echo '			<input type="text" name="zn_mc_email" class="nl-email form-control kl-newsletter-field"
			value="" placeholder="'.esc_attr(__('your.address@email.com', 'zn_framework')).'" />';
					echo '			<input type="hidden" name="zn_list_class" class="nl-lid" value="' . esc_attr($cs_lsit_id) . '" />';
					echo '			<input type="submit" name="submit"
			class="nl-submit kl-newsletter-submit kl-font-alt btn btn-fullcolor" value="' . esc_html(__( 'JOIN US', 'zn_framework' )) . '" />';
					echo '<input type="hidden" class="zn_hg_mailchimp" value="'.esc_attr($nonce).'"/>';
					echo '		</form>';
