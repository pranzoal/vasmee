...
class TH_PortfolioSlider extends ZnElements
{
	...
	function element()
	{
		...
		$classes[] = 'kl-slideshow portfolio-slider__sideshow gradient';
		$classes[] = $uid = $this->data['uid'];
		$classes[] = zn_get_element_classes($options);
		$classes[] = 'uh_' .$this->opt('ps_header_style', '');
				...
				<div class="psl__inner">
					...
					<div class="kl-slideshow-safepadding psl__container">
						...
						$slick_attributes = array(
							...
							"slidesToShow" => 1,
							"slidesToScroll" => 1,
							"autoplay" => $this->opt('enable_autoplay', 'no') == 'yes' ? true : false,
							"autoplaySpeed" => $this->opt('autoplay_timeout', 5000),
							"speed" => 1000,
							"fade" => true,
							"arrows" => true,
						...
						<div class="psl-carousel__wrapper">
							<div class="psl-carousel__container js-slick" data-slick='<?php echo json_encode($slick_attributes) ?>' >
								<?php
								if ( isset ( $options['single_pslides'] ) && is_array( $options['single_pslides'] ) ) {
									foreach ( $options['single_pslides'] as $k => $slide ) {

										echo '<div class="psl-carousel__item psl--item-'.$k.' '.($k == 0 ? 'slick-item--activated':'').'" '.WpkPageHelper::zn_schema_markup('creative_work').'>';
	...
	function options()
	{
		...
		$options = array(
			...
			'general' => array(
				...
				'options' => array(
					...
					array (
						...
						"description" => __( "Enter a numeric value automatic timeout between slides (default: 5000 milliseconds)", 'zn_framework' ),
						"id"          => "autoplay_timeout",
						"std"         => "5000",
						"type"        => "text",
						"class"       => "zn_input_xs",
			...
			'help' => znpb_get_helptab( array(
				'video'   => sprintf( '%s', esc_url('https://my.hogash.com/video_category/kallyas-wordpress-theme/#1P0fu6T1GrU') ),
				'docs'    => sprintf( '%s', esc_url('https://my.hogash.com/documentation/portfolio-slider/') ),
				'copy'    => $uid,
				'general' => true,
			)),
