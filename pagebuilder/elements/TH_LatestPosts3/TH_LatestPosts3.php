...
class TH_LatestPosts3 extends ZnElements
{
	...
	function element()
	{
		...
		?>
			...
			<div class=" latest_posts style2 latest_posts--style3 latest_posts3 <?php echo implode(' ', $elm_classes); ?>" <?php echo zn_get_element_attributes($options); ?>>
				...
				<ul class="posts latest_posts3-posts">
					...
					$theQuery = array (
						...
						'cat' => $blog_category,
						'orderby' => ( isset($options['lp_orderby_select']) ? esc_attr($options['lp_orderby_select']) : 'date' ),
						'order' => ( isset($options['lp_sort_type_select']) ? esc_attr($options['lp_sort_type_select']) : 'DESC' ),
					);
					// @since v4.1.6
